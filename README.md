...
**Diagram — non-overlapping, position-indexed chunks; a character at position `p` affects *only* token `⌊p/4⌋`:**
...
2. **Position-and-order fidelity** — a change at position `p` perturbs only tokens `⌊p/4⌋` and later (guaranteed: non-overlapping position-indexed chunks).
...
- **Tokenise** (`n=4`): `t₀ = FNV32("ABCD") = 3541452541`, `t₁ = FNV32("EFGH") = 224681381`. So `T = 2` tokens, `n_full = ⌊2/16⌋ = 0` full blocks, `tail = 2` tokens.
...
- **Tokenise:** `T = 577` ⇒ `n_full = ⌊577/16⌋ = 36` full blocks, `tail = 577 − 36·16 = 1` token.
...
| Split | slice into ⌊T/B⌋ blocks | O(T) | O(T) |
...
| I7 | Range | every ratio ∈ `[0,1]`, every count ≥ 0, no NaN across 3000 calls |
...
`buildSegmentInfo` maps blocks to message regions (`user[2]`, `tool:<name>`, `system`…). The branch condition is `previous exists ∧ previous.blockHashes.length > 0 ∧ matched < n_full`. Then:
